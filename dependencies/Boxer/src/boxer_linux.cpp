...
Selection show(const char *message, const char *title, Style style, Buttons buttons) {
   ...
   Selection selection = Selection::Error;

   if (buttons == Buttons::AbortRetryIgnore || buttons == Buttons::CancelTryContinue) {
      ...
      if (buttons == Buttons::AbortRetryIgnore) {
         gtk_dialog_add_buttons(dialog, "_Abort", Buttons::Abort, "_Retry", Buttons::Retry, "_Ignore", Buttons::Ignore);
      } else {
         gtk_dialog_add_buttons(dialog, "C_ancel", Buttons::Cancel, "_Try", Buttons::Try, "_Continue", Buttons::Continue);
      }
   ...
   selection = getSelection(gtk_dialog_run(GTK_DIALOG(dialog)));
