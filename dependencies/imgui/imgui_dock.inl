	...
	bool tabbar(Dock& dock, bool close_button)
	{
		...
		if (BeginChild(tmp, size, true))
		{
			...
			while (dock_tab)
			{
				...
				ImVec2 size_2(CalcTextSize(dock_tab->label, text_end).x, line_height);
				if (InvisibleButton(dock_tab->label, size_2))
				{
					dock_tab->setActive();
				}
				...
				{
					size_2.x += 16 + GetStyle().ItemSpacing.x;
					SameLine();
				...
				tab_base = pos.y;
				draw_list->PathClear();
				draw_list->PathLineTo(pos + ImVec2(-15, size_2.y));
				draw_list->PathBezierCurveTo(
					pos + ImVec2(-10, size_2.y), pos + ImVec2(-5, 0), pos + ImVec2(0, 0), 10);
				draw_list->PathLineTo(pos + ImVec2(size_2.x, 0));
				draw_list->PathBezierCurveTo(pos + ImVec2(size_2.x + 5, 0),
					pos + ImVec2(size_2.x + 10, size_2.y),
					pos + ImVec2(size_2.x + 15, size_2.y),
					10);
				draw_list->PathFill(
