...
ILboolean ilIsValidPsd(ILconst_string FileName)
{
	...
	PsdFile = gIO.openReadOnly(FileName);
	if (PsdFile == NULL) {
...
{
	gIO.read(gIO.handle, Header->Signature, 1, 4);
	Header->Version = GetBigUShort();
	gIO.read(gIO.handle, Header->Reserved, 1, 6);
...
ILboolean iIsValidPsd()
{
	...
	iGetPsdHead(&Head);
	gIO.seek(gIO.handle, -(ILint)sizeof(PSDHEAD), IL_SEEK_CUR);
...
ILboolean ReadGrey(ILimage* image, PSDHEAD *Head)
{
	...
	gIO.seek(gIO.handle, ColorMode, IL_SEEK_CUR);
	...
	if (gIO.read(gIO.handle, Resources, 1, ResourceSize) != ResourceSize)
		goto cleanup_error;

	MiscInfo = GetBigUInt();
	gIO.seek(gIO.handle, MiscInfo, IL_SEEK_CUR);
...
ILboolean ReadIndexed(ILimage* image, PSDHEAD *Head)
{
	...
	if (Palette == NULL)
		return IL_FALSE;
	if (gIO.read(gIO.handle, Palette, 1, ColorMode) != ColorMode)
		goto cleanup_error;
...
ILuint *GetCompChanLen(PSDHEAD *Head)
{
	...
	if (gIO.read(gIO.handle, RleTable, sizeof(ILushort), Head->Height * ChannelNum) != Head->Height * ChannelNum) {
		ifree(RleTable);
		ifree(ChanLen);
		return NULL;
	}
...
static ILuint ReadCompressedChannel(const ILuint ChanLen, ILuint Size, ILubyte* Channel)
{
	...
	for (i = 0; i < Size; ) {
		...
		if (HeadByte >= 0) {  //  && HeadByte <= 127
			...
			if (gIO.read(gIO.handle, Channel + i, HeadByte + 1, 1) != 1)
			{
...
ILboolean PsdGetData(ILimage* image, PSDHEAD *Head, void *Buffer, ILboolean Compressed)
{
	...
	if (!Compressed) {
		if (image->Bpc == 1) {
			for (c = 0; c < NumChan; c++) {
				i = 0;
				if (gIO.read(gIO.handle, Channel, Head->Width * Head->Height, 1) != 1) {
					ifree(Channel);
					return IL_FALSE;
				}
			...
			for (c = 0; c < NumChan; c++) {
				i = 0;
				if (gIO.read(gIO.handle, Channel, Head->Width * Head->Height * 2, 1) != 1) {
					ifree(Channel);
					return IL_FALSE;
				}
				image->Bps /= 2;
...
ILboolean GetSingleChannel(ILimage* image, PSDHEAD *Head, ILubyte *Buffer, ILboolean Compressed)
{
	...
	if (!Compressed) {
		if (image->Bpc == 1) {
			if (gIO.read(gIO.handle, Buffer, Head->Width * Head->Height, 1) != 1)
				return IL_FALSE;
		}
		else {  // image->Bpc == 2
			if (gIO.read(gIO.handle, Buffer, Head->Width * Head->Height * 2, 1) != 1)
				return IL_FALSE;
		}
	}
	else {
		for (i = 0; i < Head->Width * Head->Height; ) {
			...
			if (HeadByte >= 0) {  //  && HeadByte <= 127
				if (gIO.read(gIO.handle, Buffer + i, HeadByte + 1, 1) != 1)
					return IL_FALSE;
				i += HeadByte + 1;
			}
			if (HeadByte >= -127 && HeadByte <= -1) {
...
ILboolean iSavePsdInternal(ILimage* image)
{
	...
	gIO.write(Signature, 1, 4, gIO.handle);
	SaveBigShort(1);
	SaveBigInt(0);
	SaveBigShort(0);
	...
	if (TempImage->Format == IL_COLOUR_INDEX) {
		...
		for (c = 0; c < 3; c++) {
			for (i = c; i < TempPal->PalSize; i += 3) {
				gIO.putc(TempPal->Palette[i], gIO.handle);
			}
		}
	...
	if (TempImage->Bpc == 1) {
		for (c = 0; c < TempImage->Bpp; c++) {
			for (i = c; i < TempImage->SizeOfPlane; i += TempImage->Bpp) {
				gIO.putc(TempData[i], gIO.handle);
			}
		}
	}
