...
{
	//if (gIO.read(gIO.handle, FileInfo, 768, 1) != 1)
	//	return IL_FALSE;

	FileInfo->MagicNum = GetBigUInt();
	FileInfo->Offset = GetBigUInt();
	gIO.read(gIO.handle, FileInfo->Vers, 8, 1);
	...
	gIO.read(gIO.handle, FileInfo->FileName, 100, 1);
	gIO.read(gIO.handle, FileInfo->CreateTime, 24, 1);
	gIO.read(gIO.handle, FileInfo->Creator, 100, 1);
	gIO.read(gIO.handle, FileInfo->Project, 200, 1);
	if (gIO.read(gIO.handle, FileInfo->Copyright, 200, 1) != 1)
		return IL_FALSE;
	FileInfo->Key = GetBigUInt();
	gIO.seek(gIO.handle, 104, IL_SEEK_CUR);  // Skip reserved section.
...
ILboolean GetImageElement(DPX_IMAGE_ELEMENT *ImageElement)
{
	...
	gIO.read(gIO.handle, &ImageElement->RefLowQuantity, 1, 4);
	ImageElement->RefHighData = GetBigUInt();
	gIO.read(gIO.handle, &ImageElement->RefHighQuantity, 1, 4);
	ImageElement->Descriptor = gIO.getc(gIO.handle);
	ImageElement->Transfer = gIO.getc(gIO.handle);
	ImageElement->Colorimetric = gIO.getc(gIO.handle);
	ImageElement->BitSize = gIO.getc(gIO.handle);
	...
	if (gIO.read(gIO.handle, ImageElement->Description, 32, 1) != 1)
		return IL_FALSE;
...
ILboolean DpxGetImageInfo(DPX_IMAGE_INFO *ImageInfo)
{
	...
	//if (gIO.read(gIO.handle, ImageInfo, sizeof(DPX_IMAGE_INFO), 1) != 1)
	//	return IL_FALSE;
	...
	gIO.seek(gIO.handle, 52, IL_SEEK_CUR);  // Skip padding bytes.
...
ILboolean DpxGetImageOrient(DPX_IMAGE_ORIENT *ImageOrient)
{
	...
	gIO.read(gIO.handle, &ImageOrient->XCenter, 4, 1);
	gIO.read(gIO.handle, &ImageOrient->YCenter, 4, 1);
	ImageOrient->XOrigSize = GetBigUInt();
	ImageOrient->YOrigSize = GetBigUInt();
	gIO.read(gIO.handle, ImageOrient->FileName, 100, 1);
	gIO.read(gIO.handle, ImageOrient->CreationTime, 24, 1);
	gIO.read(gIO.handle, ImageOrient->InputDev, 32, 1);
	if (gIO.read(gIO.handle, ImageOrient->InputSerial, 32, 1) != 1)
		return IL_FALSE;
	...
	gIO.seek(gIO.handle, 28, IL_SEEK_CUR);  // Skip reserved bytes.
...
//	BITFILE		*File;
	...
	gIO.seek(gIO.handle, ImageInfo.ImageElement[CurElem].DataOffset, IL_SEEK_SET);
...
//@TODO: Deal with different origins!
	...
	switch (ImageInfo.ImageElement[CurElem].BitSize)
	{
		...
		case 32:
			...
			if (gIO.read(gIO.handle, image->Data, image->SizeOfData, 1) != 1)
				return IL_FALSE;
			goto finish;
	}
