...
/*ILboolean ilLoadPxr(ILconst_string FileName)
{
	...
	PxrFile = gIO.openReadOnly(FileName);
	if (PxrFile == NULL) {
...
ILboolean iLoadPxrInternal()
{
	...
	gIO.seek(gIO.handle, 416, IL_SEEK_SET);
	Height = GetLittleUShort();
	Width = GetLittleUShort();
	gIO.seek(gIO.handle, 424, IL_SEEK_SET);
	Bpp = (ILubyte)gIO.getc(gIO.handle);
	...
	gIO.seek(gIO.handle, 1024, IL_SEEK_SET);
	gIO.read(gIO.handle, iCurImage->Data, 1, iCurImage->SizeOfData);
