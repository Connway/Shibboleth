...
ILboolean ilIsValidIcns(ILconst_string FileName)
{
	...
	IcnsFile = gIO.openReadOnly(FileName);
	if (IcnsFile == NULL) {
...
ILboolean iIsValidIcns()
{
	...
	ILint read = gIO.read(gIO.handle, Header.Head, 1, 4);
...
ILboolean iLoadIcnsInternal(ILimage* image)
{
	...
	gIO.read(gIO.handle, Header.Head, 4, 1);
	Header.Size = GetBigInt();

	if (strncmp(Header.Head, "icns", 4))  // First 4 bytes have to be 'icns'.
		return IL_FALSE;

	while ((ILint)gIO.tell(gIO.handle) < Header.Size && !gIO.eof(gIO.handle))
	{
		gIO.read(gIO.handle, Entry.ID, 4, 1);
		...
		{
			gIO.seek(gIO.handle, Entry.Size - 8, IL_SEEK_CUR);
		}
	}
...
ILboolean iIcnsReadData(ILimage* image, ILboolean *BaseCreated, ILboolean IsAlpha, ILint Width, ICNSDATA *Entry, ILimage **Image)
{
	...
	{
		gIO.read(gIO.handle, Data, Entry->Size - 8, 1);  // Size includes the header
