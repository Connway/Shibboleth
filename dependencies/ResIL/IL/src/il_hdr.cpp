...
ILboolean ilIsValidHdr(ILconst_string FileName)
{
	...
	HdrFile = gIO.openReadOnly(FileName);
	if (HdrFile == NULL) {
...
ILboolean iGetHdrHead(HDRHEADER *Header)
{
	...
	gIO.read(gIO.handle, Header->Signature, 1, 10);
	...
	if (gIO.read(gIO.handle, &a, 1, 1) != 1)
		return IL_FALSE;

	while (!done) {
		if (gIO.read(gIO.handle, &b, 1, 1) != 1)
			return IL_FALSE;
		if (b == '\n' && a == '\n')
			done = IL_TRUE;
		else
...
ILboolean iIsValidHdr()
{
	...
	Read = gIO.read(gIO.handle, Head, 1, 10);
...
ILboolean iLoadHdrInternal(ILimage* image)
{
	...
	if (!iCheckHdr(&Header)) {
		//gIO.seek(gIO.handle, -(ILint)sizeof(HDRHEAD), IL_SEEK_CUR);
		ilSetError(IL_INVALID_FILE_HEADER);
		return IL_FALSE;
	}
...
void ReadScanline(ILubyte *scanline, ILuint w) {
	...
	r = gIO.getc(gIO.handle);
	g = gIO.getc(gIO.handle);
	b = gIO.getc(gIO.handle);
	e = gIO.getc(gIO.handle);
	...
	if (r == 2 && g == 2) {
		...
		for (k = 0; k < 4; ++k) {
			...
			j = 0;
			while (j < length) {
				t = gIO.getc(gIO.handle);
				if (t > 128) { //Run?
					ILubyte val = gIO.getc(gIO.handle);
					t &= 127;
					//copy current byte
					while (t > 0 && j < length) {
						*runner = val;
...
/*ILboolean ilSaveHdr(ILimage* image, const ILstring FileName)
{
	...
	HdrFile = gIO.openWrite(FileName);
	if (HdrFile == NULL) {
		ilSetError(IL_COULD_NOT_OPEN_FILE);
		return IL_FALSE;
	}
...
int RGBE_WritePixels(float *data, int numpixels)
{
	...
	while (numpixels-- > 0) {
		...
		data += RGBE_DATA_SIZE;
		if (gIO.write(rgbe, sizeof(rgbe), 1, gIO.handle) < 1)
			return IL_FALSE;
	}
	return IL_TRUE;
}
...
#define MINRUNLENGTH 4
	...
	while (cur < numbytes) {
		...
		if ((old_run_count > 1)&&(old_run_count == beg_run - cur)) {
			...
			buf[1] = data[cur];
			if (gIO.write(buf,sizeof(buf[0])*2,1, gIO.handle) < 1)
				return IL_FALSE;
			cur = beg_run;
		}
		...
		while(cur < beg_run) {
			...
			if (nonrun_count > 128) 
				nonrun_count = 128;
			buf[0] = nonrun_count;
			if (gIO.write(buf,sizeof(buf[0]),1, gIO.handle) < 1)
				return IL_FALSE;
			if (gIO.write(&data[cur],sizeof(data[0])*nonrun_count,1, gIO.handle) < 1)
				return IL_FALSE;
			cur += nonrun_count;
		}
