...
BITFILE *bfile(ILHANDLE File)
{
	...
	if (File != NULL) {
		...
		if (ToReturn != NULL) {
			ToReturn->File = File;
			ToReturn->BitPos = gIO.tell(gIO.handle) << 3;
			ToReturn->ByteBitOff = 8;
			ToReturn->Buff = 0;
		}
	}
...
ILint bseek(BITFILE *BitFile, ILuint Offset, ILuint Mode)
{
	...
	{
		case IL_SEEK_SET:
			if (!gIO.seek(gIO.handle, Offset >> 3, Mode)) {
		...
		case IL_SEEK_CUR:
			...
			break;
		case IL_SEEK_END:
			KeepPos = gIO.tell(gIO.handle);
			gIO.seek(gIO.handle, 0, IL_SEEK_END);
			Len = gIO.tell(gIO.handle);
...
ILint bread(void *Buffer, ILuint Size, ILuint Number, BITFILE *BitFile)
{
	...
	while (BuffPos < Count) {
		if (BitFile->ByteBitOff < 0 || BitFile->ByteBitOff > 7) {
			...
			if (gIO.read(gIO.handle, &BitFile->Buff, 1, 1) != 1)  // Reached eof or error...
				return BuffPos;
		}
