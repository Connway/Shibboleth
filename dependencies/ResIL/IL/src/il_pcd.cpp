...
ILboolean iLoadPcdInternal(ILimage* image)
{
	...
	gIO.seek(gIO.handle, 72, IL_SEEK_CUR);
	if (gIO.read(gIO.handle, &VertOrientation, 1, 1) != 1)
		return IL_FALSE;

	gIO.seek(gIO.handle, -72, IL_SEEK_CUR);  // Can't rewind
	...
	{
		case 0:
			gIO.seek(gIO.handle, 0x02000, IL_SEEK_CUR);
			Width = 192;
			Height = 128;
			break;
		case 1:
			gIO.seek(gIO.handle, 0x0b800, IL_SEEK_CUR);
			Width = 384;
			Height = 256;
			break;
		case 2:
			gIO.seek(gIO.handle, 0x30000, IL_SEEK_CUR);
			Width = 768;
			Height = 512;
			break;
		default:
	...
	if (gIO.tell(gIO.handle) == IL_EOF)  // Supposed to have data here.
		return IL_FALSE;
	...
	Total = Height >> 1;
	for (i = 0; i < Total; i++) {
		gIO.read(gIO.handle, Y1, 1, Width);
		gIO.read(gIO.handle, Y2, 1, Width);
		if (gIO.read(gIO.handle, CbCr, 1, Width) != Width) {  // Only really need to check the last one.
			ifree(Y1);
			ifree(Y2);
			ifree(CbCr);
			return IL_FALSE;
