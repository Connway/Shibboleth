...
ILboolean ilIsValidJp2(ILconst_string FileName)
{
	...
	Jp2File = gIO.openReadOnly(FileName);
	if (Jp2File == NULL) {
...
ILboolean iIsValidJp2(void)
{
	...
	gIO.seek(gIO.handle, 4, IL_SEEK_CUR);  // Skip the 4 bytes that tell the size of the signature box.
	if (gIO.read(gIO.handle, Signature, 1, 4) != 4) {
		gIO.seek(gIO.handle, -4, IL_SEEK_CUR);
		return IL_FALSE;  // File read error
	}

	gIO.seek(gIO.handle, -8, IL_SEEK_CUR);  // Restore to previous state
...
{
	obj;
	return gIO.read(gIO.handle, buf, 1, cnt);
}

static int iJp2_file_write(jas_stream_obj_t *obj, char *buf, int cnt)
{
	obj;
	return gIO.write(buf, 1, cnt, gIO.handle);
}
...
static long iJp2_file_seek(jas_stream_obj_t *obj, long offset, int origin)
{
	...
	{
		case SEEK_SET:
			return gIO.seek(gIO.handle, offset, IL_SEEK_SET);
		case SEEK_CUR:
			return gIO.seek(gIO.handle, offset, IL_SEEK_CUR);
		case SEEK_END:
			return gIO.seek(gIO.handle, offset, IL_SEEK_END);
	}
	return 0;  // Failed
}
...
/*ILboolean ilSaveJp2(ILimage* image, const ILstring FileName)
{
	...
	Jp2File = gIO.openWrite(FileName);
	if (Jp2File == NULL) {
		ilSetError(IL_COULD_NOT_OPEN_FILE);
		return IL_FALSE;
	}
