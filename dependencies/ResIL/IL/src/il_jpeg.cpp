...
ILboolean iIsValidJpeg()
{
	...
	iGetJpgHead(Head);
	gIO.seek(gIO.handle, -2, IL_SEEK_CUR);  // Go ahead and restore to previous state
...
ILboolean ilIsValidJpeg(ILconst_string FileName)
{
	...
	JpegFile = gIO.openReadOnly(FileName);
	if (JpegFile == NULL) {
...
fill_input_buffer (j_decompress_ptr cinfo)
{
	...
	nbytes = gIO.read(gIO.handle, src->buffer, 1, INPUT_BUF_SIZE);
...
{
	iwrite_ptr dest = (iwrite_ptr)cinfo->dest;
	gIO.write(dest->buffer, 1, OUTPUT_BUF_SIZE, gIO.handle);
	dest->pub.next_output_byte = dest->buffer;
	dest->pub.free_in_buffer = OUTPUT_BUF_SIZE;
	return IL_TRUE;
}
...
{
	iwrite_ptr dest = (iwrite_ptr)cinfo->dest;
	gIO.write(dest->buffer, 1, OUTPUT_BUF_SIZE - (ILuint)dest->pub.free_in_buffer, gIO.handle);
	return;
}
