...
ILenum ILAPIENTRY ilDetermineType(ILconst_string FileName)
{
	...
	if (FileName != NULL) {
		...
		ilResetRead();
		gIO.handle = gIO.openReadOnly(FileName);
		if (gIO.handle != NULL) {
			Type = ilDetermineTypeFuncs();
			gIO.close(gIO.handle);
		}
...
ILenum ILAPIENTRY ilDetermineTypeFuncs()
{
	...
	ILubyte buf[bufSize];
	ILint64 read = gIO.read(gIO.handle, buf, 1, bufSize);
...
ILboolean ILAPIENTRY ilLoadFuncs2(ILimage* image, ILenum type)
{
	switch (type)
	{
		...
		case IL_TEXTURE:
			...
			gIO.seek(gIO.handle, 48, IL_SEEK_CUR);
			return iLoadDdsInternal(image);
		#endif
...
ILboolean ILAPIENTRY ilSaveFuncs2(ILimage* image, ILenum type)
{
	...
	#ifndef IL_NO_VTF
	case IL_VTF:
		bRet = iSaveVtfInternal();
		break;
	#endif

	#ifndef IL_NO_WBMP
	case IL_WBMP:
		bRet = iSaveWbmpInternal();
		break;
	#endif

	// Check if we just want to save the palette.
	// @todo: must be ported to use gIO
/*	case IL_JASC_PAL:
...
ILboolean ILAPIENTRY ilSave(ILenum type, ILconst_string FileName)
{
	...
	ilResetWrite();
	gIO.handle = gIO.openWrite(FileName);
	if (gIO.handle != NULL) {
...
ILuint ILAPIENTRY ilSaveL(ILenum Type, void *Lump, ILuint Size)
{
	...
	ILint64 pos1 = gIO.tell(gIO.handle);
	ILboolean bRet = ilSaveFuncs2(iCurImage, Type);
	ILint64 pos2 = gIO.tell(gIO.handle);
