...
ILboolean ilIsValidMp3(ILconst_string FileName)
{
	...
	Mp3File = gIO.openReadOnly(FileName);
	if (Mp3File == NULL) {
...
{
	if (gIO.read(gIO.handle, Header->Signature, 3, 1) != 1)
		return IL_FALSE;
	Header->VersionMajor = gIO.getc(gIO.handle);
	Header->VersionMinor = gIO.getc(gIO.handle);
	Header->Flags = gIO.getc(gIO.handle);
...
ILuint iFindMp3Pic(MP3HEAD *Header)
{
	...
	do {
		if (gIO.read(gIO.handle, ID, 4, 1) != 1)
			return MP3_NONE;
		...
		if (!strncmp(ID, "APIC", 4)) {
			...
			TextEncoding = gIO.getc(gIO.handle);
			// Get the MimeType (read until we hit 0).
			for (i = 0; i < 65; i++) {
				MimeType[i] = gIO.getc(gIO.handle);
				if (MimeType[i] == 0)
					break;
			}
			...
			PicType = gIO.getc(gIO.handle);  // Whether this is a cover, band logo, etc.

			// Skip the description.
			for (i = 0; i < 65; i++) {
				Description[i] = gIO.getc(gIO.handle);
				if (Description[i] == 0)
					break;
			}
			if (Description[i] != 0)
			...
			gIO.seek(gIO.handle, FrameSize, IL_SEEK_CUR);
		}

		//if (!strncmp(MimeType, "
	} while (!gIO.eof(gIO.handle) && gIO.tell(gIO.handle) < Header->Length);
