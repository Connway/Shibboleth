...
ILboolean ilIsValidBmp(ILconst_string CONST_RESTRICT FileName)
{
	...
	BitmapFile = gIO.openReadOnly(FileName);
	if (BitmapFile == NULL) {
...
{
	ILint64 read = gIO.read(gIO.handle, header, 1, sizeof(BMPHEAD));
	gIO.seek(gIO.handle, -read, IL_SEEK_CUR);  // Go ahead and restore to previous state
...
ILboolean iGetOS2Head(OS2_HEAD * const Header)
{
	...
	ILint64 read = gIO.read(gIO.handle, Header, 1, toRead);
...
ILboolean iGetOS2Bmp(ILimage* image, OS2_HEAD *Header)
{
	...
	if (Header->cBitCount == 1) {
		...
		if (gIO.read(gIO.handle, image->Pal.Palette, 1, 2 * 3) != 6)
			return IL_FALSE;

		PadSize = ((32 - (image->Width % 32)) / 8) % 4;  // Has to truncate.
		gIO.seek(gIO.handle, Header->DataOff, IL_SEEK_SET);

		for (j = 0; j < image->Height; j++) {
			Read = 0;
			for (i = 0; i < image->Width; ) {
				if (gIO.read(gIO.handle, &ByteData, 1, 1) != 1)
					return IL_FALSE;
				Read++;
				k = 128;
				for (c = 0; c < 8; c++) {
			...
			gIO.seek(gIO.handle, PadSize, IL_SEEK_CUR);
		}
		return IL_TRUE;
	}
	...
	if (Header->cBitCount == 4) {
		...
		if (gIO.read(gIO.handle, image->Pal.Palette, 1, 16 * 3) != 16*3)
			return IL_FALSE;
	...
	if (Header->cBitCount == 8) {
		...
		if (gIO.read(gIO.handle, image->Pal.Palette, 1, 256 * 3) != 256*3)
			return IL_FALSE;
	}
	...
	if (PadSize == 0) {
		...
		for (i = 0; i < image->Height; i++) {
			if (gIO.read(gIO.handle, image->Data + i * image->Bps, 1, image->Bps) != image->Bps)
				return IL_FALSE;
			gIO.seek(gIO.handle, PadSize, IL_SEEK_CUR);
		}
	}
...
ILboolean ilReadUncompBmp1(ILimage* image, BMPHEAD * Header)
{
	...
	// Read the palette
	gIO.seek(gIO.handle, sizeof(BMPHEAD), IL_SEEK_SET);
	...
	gIO.seek(gIO.handle, Header->bfDataOff, IL_SEEK_SET);
	...
	for (j = 0; j < image->Height; j++) {
		...
		//gIO.seek(gIO.handle, PadSize, IL_SEEK_CUR);
		gIO.read(gIO.handle, Padding, 1, PadSize);
	}
...
ILboolean ilReadUncompBmp8(ILimage* image, BMPHEAD * Header)
{
	...
	if (PadSize == 0) {
		...
		if (gIO.read(gIO.handle, image->Data, 1, image->SizeOfPlane) != image->SizeOfPlane)
			return IL_FALSE;
	} else {	
		// Remove padding while reading
		for (ILuint i = 0; i < image->SizeOfPlane; i += image->Bps) {
			ILuint read = gIO.read(gIO.handle, image->Data + i, 1, image->Bps);
			if (read != image->Bps) {
				return IL_FALSE;
			}
			gIO.seek(gIO.handle, PadSize, IL_SEEK_CUR);
...
ILboolean ilReadUncompBmp16(ILimage* image, BMPHEAD * Header)
{
	...
	if (Header->biCompression == 3)
	{
		...
		gIO.seek(gIO.handle, readOffset, IL_SEEK_SET); //seek to bitfield data
		gIO.read(gIO.handle, &rMask, 4, 1);
		gIO.read(gIO.handle, &gMask, 4, 1);
		gIO.read(gIO.handle, &bMask, 4, 1);
		UInt(&rMask);
		UInt(&gMask);
		UInt(&bMask);
	...
	for (ILuint y = 0; y < image->Height; y++) {
		gIO.seek(gIO.handle, inputOffset, IL_SEEK_SET);
		ILuint read = gIO.read(gIO.handle, inputScanline, 1, inputBps);
		if (read != inputBps) {
			return IL_FALSE;
		}
...
ILboolean ilReadUncompBmp32(ILimage* image, BMPHEAD * Header)
{
	...
	// Read pixel data
	gIO.seek(gIO.handle, Header->bfDataOff, IL_SEEK_SET);
	ILuint read = gIO.read(gIO.handle, image->Data, 1, image->SizeOfPlane);
...
ILboolean ilReadRLE8Bmp(ILimage* image, BMPHEAD *Header)
{
	...
	// Read the palette
	gIO.seek(gIO.handle, sizeof(BMPHEAD), IL_SEEK_SET);
	if (gIO.read(gIO.handle, image->Pal.Palette, image->Pal.PalSize, 1) != 1)
		return IL_FALSE;
    ...
    while (offset < image->SizeOfData) {
		if (gIO.read(gIO.handle, Bytes, sizeof(Bytes), 1) != 1)
			return IL_FALSE;
		if (Bytes[0] == 0x00) {  // Escape sequence
			switch (Bytes[1])
			{
				...
				default:
					...
					if (gIO.read(gIO.handle, image->Data + offset, (ILuint)count, 1) != 1)
						return IL_FALSE;
					offset += count;
					if ((count & 1) == 1)  // Must be on a word boundary
						if (gIO.read(gIO.handle, Bytes, 1, 1) != 1)
							return IL_FALSE;
					break;
			}
		} else {
...
ILboolean ilReadRLE4Bmp(ILimage* image, BMPHEAD *Header)
{
	...
	while (offset < image->SizeOfData) {
      int align;
		if (gIO.read(gIO.handle, &Bytes[0], sizeof(Bytes), 1) != 1)
			return IL_FALSE;
         ...
         default:	  // Run of pixels
            count = IL_MIN (Bytes[1], image->SizeOfData-offset);
				...
				for (i = 0; i < count; i++) {
					...
					if ((i & 0x01) == 0) {
						if (gIO.read(gIO.handle, &Bytes[0], sizeof(Bytes[0]), 1) != 1)
							return IL_FALSE;
						byte = (Bytes[0] >> 4);
					}
					else
...
ILboolean iSaveBitmapInternal(ILimage* image)
{
	...
	gIO.putc('B', gIO.handle);  // Comprises the
	gIO.putc('M', gIO.handle);  //  "signature"
	...
	gIO.write(TempPal->Palette, 1, TempPal->PalSize, gIO.handle);
	...
	if( compress_rle8 == IL_TRUE ) {
		...
		gIO.write(Dest,1,FileSize, gIO.handle);
	} else {
		PadSize = (4 - (TempImage->Bps % 4)) % 4;
		// No padding, so write data directly.
		if (PadSize == 0) {
			gIO.write(TempData, 1, TempImage->SizeOfPlane, gIO.handle);
		} else {  // Odd width, so we must pad each line.
			for (i = 0; i < TempImage->SizeOfPlane; i += TempImage->Bps) {
				gIO.write(TempData + i, 1, TempImage->Bps, gIO.handle); // Write data
				gIO.write(&Padding, 1, PadSize, gIO.handle); // Write pad byte(s)
			}
		}
	}
	...
	gIO.seek(gIO.handle, 2, IL_SEEK_SET);
	...
	gIO.seek(gIO.handle, FileSize, IL_SEEK_SET);
