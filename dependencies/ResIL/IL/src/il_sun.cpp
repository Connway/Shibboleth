...
ILboolean ilIsValidSun(ILconst_string FileName)
{
	...
	SunFile = gIO.openReadOnly(FileName);
	if (SunFile == NULL) {
...
ILboolean iIsValidSun()
{
	...
	if (!iGetSunHead(&Head))
		return IL_FALSE;
	gIO.seek(gIO.handle, -(ILint)sizeof(SUNHEAD), IL_SEEK_CUR);
	...
	switch (Header.Depth)
	{
		case 1:  //@TODO: Find a file to test this on.
			...
			if (Header.ColorMapLength == 0) {  // Create the color map
				...
				gIO.read(gIO.handle, iCurImage->Pal.Palette, 1, 6);  // Read in the color map.
			}
		...
		case 8:
			if (Header.ColorMapType == IL_SUN_NO_MAP) {  // Greyscale image
				...
				if (iCurImage->Pal.Palette == NULL)
					return IL_FALSE;
				if (gIO.read(gIO.handle, iCurImage->Pal.Palette, 1, Header.ColorMapLength) != Header.ColorMapLength) {  // Read color map.
			...
			if (Header.Type != IL_SUN_BYTE_ENC) {  // Regular uncompressed image data
				...
				for (i = 0; i < Header.Height; i++) {
					gIO.read(gIO.handle, iCurImage->Data + i * Header.Width, 1, iCurImage->Bps);
				...
				for (i = 0; i < iCurImage->Height; i++) {
					...
					if (BytesRead % 2)  // Each scanline must be aligned on a 2-byte boundary.
						gIO.getc(gIO.handle);  // Skip padding
				}
			}
			break;
		...
		case 24:
			if (Header.ColorMapLength > 0)  // Ignore any possible colormaps.
				gIO.seek(gIO.handle, Header.ColorMapLength, IL_SEEK_CUR);
			...
			if (Header.Type != IL_SUN_BYTE_ENC) {  // Regular uncompressed image data
				Padding = (2 - (iCurImage->Bps % 2)) % 2;  // Must be padded on a 16-bit boundary (2 bytes)
				for (i = 0; i < Header.Height; i++) {
					gIO.read(gIO.handle, iCurImage->Data + i * Header.Width * 3, 1, iCurImage->Bps);
					if (Padding)  // Only possible for padding to be 0 or 1.
						gIO.getc(gIO.handle);
				}
			}
		...
		case 32:
			...
			for (i = 0; i < Header.Height; i++) {
				for (j = 0; j < Header.Width; j++) {
					gIO.getc(gIO.handle);  // There is a pad byte before each pixel.
					iCurImage->Data[Offset]   = gIO.getc(gIO.handle);
					iCurImage->Data[Offset+1] = gIO.getc(gIO.handle);
					iCurImage->Data[Offset+2] = gIO.getc(gIO.handle);
				}
			}
			break;
