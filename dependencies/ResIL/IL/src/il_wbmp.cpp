...
ILboolean iLoadWbmpInternal()
{
	...
	if (gIO.getc(gIO.handle) != 0 || gIO.getc(gIO.handle) != 0) {  // The first two bytes have to be 0 (the "header")
...
ILboolean iSaveWbmpInternal()
{
	...
	gIO.putc(0, gIO.handle);  // First two header values
	gIO.putc(0, gIO.handle);  //  must be 0.
