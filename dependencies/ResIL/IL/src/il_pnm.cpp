...
ILboolean ilIsValidPnm(ILconst_string FileName)
{
	...
	PnmFile = gIO.openReadOnly(FileName);
	if (PnmFile == NULL) {
...
ILboolean iIsValidPnm()
{
	...
	Read = gIO.read(gIO.handle, Head, 1, 2);
...
ILimage *ilReadBinaryPpm(PPMINFO *Info)
{
	...
	ILuint size = gIO.tell(gIO.handle);
	gIO.seek(gIO.handle, size-Size,IL_SEEK_SET);
	*/
	if (gIO.read(gIO.handle, iCurImage->Data, 1, Size ) != Size) {
		ilCloseImage(iCurImage);	
		return NULL;
	}
	return iCurImage;
...
ILboolean iGetWord(ILboolean final)
{
	...
	while (Looping) {
		while ((Current = gIO.getc(gIO.handle)) != IL_EOF && Current != '\n' && Current != '#' && Current != ' ') {
		...
		while ((Current = gIO.getc(gIO.handle)) != IL_EOF) {
			if (Current != ' ')
				break;
		}
...
/*ILboolean ilSavePnm(const ILstring FileName)
{
	...
	PnmFile = gIO.openWrite(FileName);
	if (PnmFile == NULL) {
		ilSetError(IL_COULD_NOT_OPEN_FILE);
		return IL_FALSE;
	}
		...
		for (j = 0; j < Bpp; j++) {
			if (Binary) {
				if (Type == IL_PBM_BINARY) {
					gIO.putc((ILubyte)(TempData[i] > 127 ? 1 : 0), gIO.handle);
				}
				else {
