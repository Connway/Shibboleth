...
ILboolean ilIsValidPsp(ILconst_string FileName)
{
	...
	PspFile = gIO.openReadOnly(FileName);
	if (PspFile == NULL) {
...
{
	if (gIO.read(gIO.handle, Header.FileSig, 1, 32) != 32)
		return IL_FALSE;
...
ILboolean iIsValidPsp()
{
	if (!iGetPspHead())
		return IL_FALSE;
	gIO.seek(gIO.handle, -(ILint)sizeof(PSPHEAD), IL_SEEK_CUR);
...
ILboolean ReadGenAttributes()
{
	...
	if (gIO.read(gIO.handle, &AttHead, sizeof(AttHead), 1) != 1)
		return IL_FALSE;
	...
	if (Header.MajorVersion != 3)
		ChunkLen -= 4;
	if (gIO.read(gIO.handle, &AttChunk, IL_MIN(sizeof(AttChunk), ChunkLen), 1) != 1)
		return IL_FALSE;

	// Can have new entries in newer versions of the spec (4.0).
	Padding = (ChunkLen) - sizeof(AttChunk);
	if (Padding > 0)
		gIO.seek(gIO.handle, Padding, IL_SEEK_CUR);
...
ILboolean ParseChunks()
{
	...
	do {
		if (gIO.read(gIO.handle, &Block, 1, sizeof(Block)) != sizeof(Block)) {
		...
		switch (Block.BlockID)
		{
			...
			//default:
				//gIO.seek(gIO.handle, Block.BlockLen, IL_SEEK_CUR);
		}

		// Skip to next block just in case we didn't read the entire block.
		gIO.seek(gIO.handle, Pos + Block.BlockLen, IL_SEEK_SET);
...
ILboolean ReadLayerBlock(ILuint BlockLen)
{
	...
	if (Header.MajorVersion == 3) {
		gIO.seek(gIO.handle, 256, IL_SEEK_CUR);  // We don't care about the name of the layer.
		gIO.read(gIO.handle, &LayerInfo, sizeof(LayerInfo), 1);
		if (gIO.read(gIO.handle, &Bitmap, sizeof(Bitmap), 1) != 1)
			return IL_FALSE;
	}
	else {  // Header.MajorVersion >= 4
		ChunkSize = GetLittleUInt();
		NumChars = GetLittleUShort();
		gIO.seek(gIO.handle, NumChars, IL_SEEK_CUR);  // We don't care about the layer's name.

		ChunkSize -= (2 + 4 + NumChars);

		if (gIO.read(gIO.handle, &LayerInfo, IL_MIN(sizeof(LayerInfo), ChunkSize), 1) != 1)
			return IL_FALSE;
...
ILboolean ReadAlphaBlock(ILuint BlockLen)
{
	...
	if (Header.MajorVersion >= 4) {
		...
		gIO.seek(gIO.handle, StringSize, IL_SEEK_CUR);
		if (gIO.read(gIO.handle, &AlphaInfo, sizeof(AlphaInfo), 1) != 1)
			return IL_FALSE;
		...
		ChunkSize = GetLittleUInt();
		if (gIO.read(gIO.handle, &AlphaChunk, sizeof(AlphaChunk), 1) != 1)
			return IL_FALSE;
		...
		gIO.seek(gIO.handle, 256, IL_SEEK_CUR);
		gIO.read(gIO.handle, &AlphaInfo, sizeof(AlphaInfo), 1);
		if (gIO.read(gIO.handle, &AlphaChunk, sizeof(AlphaChunk), 1) != 1)
			return IL_FALSE;
	}
...
ILubyte *GetChannel()
{
	...
	if (Header.MajorVersion >= 4) {
		ChunkSize = GetLittleUInt();
		if (gIO.read(gIO.handle, &Channel, sizeof(Channel), 1) != 1)
			return NULL;
	...
	if (gIO.read(gIO.handle, CompData, 1, Channel.CompLen) != Channel.CompLen) {
		ifree(CompData);
		ifree(Data);
		return NULL;
	}
...
ILboolean ReadPalette(ILuint BlockLen)
{
	...
	if (gIO.read(gIO.handle, Pal.Palette, Pal.PalSize, 1) != 1) {
		ifree(Pal.Palette);
		return IL_FALSE;
	}
