...
ILboolean ilLoadHaloPal(ILconst_string FileName)
{
	...
	HaloFile = gIO.openReadOnly(FileName);
	if (HaloFile == NULL) {
		ilSetError(IL_COULD_NOT_OPEN_FILE);
		return IL_FALSE;
	}

	if (gIO.read(gIO.handle, &HaloHead, sizeof(HALOHEAD), 1) != 1)
		return IL_FALSE;
	...
	if (gIO.read(gIO.handle, TempPal, sizeof(ILushort), Size) != Size) {
		gIO.close(HaloFile);
		ifree(TempPal);
		return IL_FALSE;
	}
...
ILboolean ilLoadColPal(ILconst_string FileName)
{
	...
	ColFile = gIO.openReadOnly(FileName);
	if (ColFile == NULL) {
		ilSetError(IL_COULD_NOT_OPEN_FILE);
		return IL_FALSE;
	}
	...
	gIO.seek(gIO.handle, 0, IL_SEEK_SET);
	...
	if (RealFileSize > 768) {  // has a header
		...
		if (gIO.read(gIO.handle, &Version, 2, 1) != 1) {
			gIO.close(ColFile);
			return IL_FALSE;
		}
		if (Version != 0xB123) {
	...
	if (gIO.read(gIO.handle, iCurImage->Pal.Palette, 1, 768) != 768) {
		gIO.close(ColFile);
		ifree(iCurImage->Pal.Palette);
		iCurImage->Pal.Palette = NULL;
		return IL_FALSE;
...
ILboolean ilLoadActPal(ILconst_string FileName)
{
	...
	ActFile = gIO.openReadOnly(FileName);
	if (ActFile == NULL) {
		ilSetError(IL_COULD_NOT_OPEN_FILE);
		return IL_FALSE;
	}
...
ILboolean ilLoadPltPal(ILconst_string FileName)
{
	...
	PltFile = gIO.openReadOnly(FileName);
	if (PltFile == NULL) {
		ilSetError(IL_COULD_NOT_OPEN_FILE);
		return IL_FALSE;
	}
	...
	if (gIO.read(gIO.handle, iCurImage->Pal.Palette, iCurImage->Pal.PalSize, 1) != 1) {
		ifree(iCurImage->Pal.Palette);
		iCurImage->Pal.Palette = NULL;
		gIO.close(PltFile);
		return IL_FALSE;
