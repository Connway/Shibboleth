...
ILboolean ilIsValidFits(ILconst_string FileName)
{
	...
	FitsFile = gIO.openReadOnly(FileName);
	if (FitsFile == NULL) {
...
//@TODO: Use something other than memset?
	...
	gIO.seek(gIO.handle, (2880 - (gIO.tell(gIO.handle) % 2880)) % 2880, IL_SEEK_CUR);
...
ILenum GetCardImage(FITSHEAD *Header)
{
	...
	if (gIO.read(gIO.handle, Buffer, 1, 80) != 80)  // Each card image is exactly 80 bytes long.
