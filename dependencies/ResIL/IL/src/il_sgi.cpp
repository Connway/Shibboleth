...
ILboolean ilIsValidSgi(ILconst_string FileName)
{
	...
	SgiFile = gIO.openReadOnly(FileName);
	if (SgiFile == NULL) {
...
ILboolean iGetSgiHead(iSgiHeader *Header)
{
	...
	Header->Storage = (ILbyte)gIO.getc(gIO.handle);
	Header->Bpc = (ILbyte)gIO.getc(gIO.handle);
	...
	gIO.read(gIO.handle, Header->Name, 1, 80);
	Header->ColMap = GetBigInt();
	gIO.read(gIO.handle, Header->Dummy, 1, 404);
...
ILboolean iIsValidSgi()
{
	...
	if (!iGetSgiHead(&Head))
		return IL_FALSE;
	gIO.seek(gIO.handle, -(ILint)sizeof(iSgiHeader), IL_SEEK_CUR);  // Go ahead and restore to previous state
...
ILboolean iReadRleSgi(iSgiHeader *Head)
{
	...
	if (OffTable == NULL || LenTable == NULL)
		goto cleanup_error;
	if (gIO.read(gIO.handle, OffTable, TableSize * sizeof(ILuint), 1) != 1)
		goto cleanup_error;
	if (gIO.read(gIO.handle, LenTable, TableSize * sizeof(ILuint), 1) != 1)
		goto cleanup_error;
	...
	for (ixPlane = 0; ixPlane < Head->ZSize; ixPlane++) {
		for (ixHeight = 0, Cur = 0;	ixHeight < Head->YSize;
			...
			gIO.seek(gIO.handle, RleOff, IL_SEEK_SET);
...
ILint iGetScanLine(ILubyte *ScanLine, iSgiHeader *Head, ILuint Length)
{
	...
	{
		Pixel = 0;
		if (gIO.read(gIO.handle, &Pixel, Head->Bpc, 1) != 1)
			return -1;
		...
		if (Pixel & 0x80) {  // If top bit set, then it is a "run"
			if (gIO.read(gIO.handle, ScanLine, Head->Bpc, Count) != Count)
				return -1;
...
ILboolean iReadNonRleSgi(iSgiHeader *Head)
{
	...
	for (c = 0; c < iCurImage->Bpp; c++) {
		for (i = c; i < iCurImage->SizeOfData; i += iCurImage->Bpp) {
			if (gIO.read(gIO.handle, iCurImage->Data + i, 1, 1) != 1) {
				return IL_FALSE;
			}
		}
	}
...
/*ILboolean ilSaveSgi(const ILstring FileName)
{
	...
	SgiFile = gIO.openWrite(FileName);
	if (SgiFile == NULL) {
		ilSetError(IL_COULD_NOT_OPEN_FILE);
		return IL_FALSE;
	}
...
ILboolean iSaveSgiInternal()
{
	...
	if (FName) {
		...
		c = c < 79 ? 79 : c;
		gIO.write(FName, 1, c, gIO.handle);
		c = 80 - c;
		for (i = 0; i < c; i++) {
			gIO.putc(0, gIO.handle);
		}
	...
	if (!Compress) {
		for (c = 0; c < Temp->Bpp; c++) {
			for (i = c; i < Temp->SizeOfData; i += Temp->Bpp) {
				gIO.putc(TempData[i], gIO.handle);  // Have to save each colour plane separately.
			}
		}
	}
	else {
		...
		ILuint bps)
{
	...
	TableOff = gIO.tell(gIO.handle);
	gIO.write(StartTable, sizeof(ILuint), h * numChannels, gIO.handle);
	gIO.write(LenTable, sizeof(ILuint), h * numChannels, gIO.handle);

	DataOff = gIO.tell(gIO.handle);
	for (c = 0; c < numChannels; c++) {
		for (y = 0; y < h; y++) {
			...
			gIO.write(CompLine, 1, *(LenTable + h * c + y), gIO.handle);
		}
	}

	gIO.seek(gIO.handle, TableOff, IL_SEEK_SET);
