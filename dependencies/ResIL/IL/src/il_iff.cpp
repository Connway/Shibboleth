...
iff_chunk iff_begin_read_chunk()
{
	...
	chunkStack[chunkDepth].start = gIO.tell(gIO.handle);
...
void iff_end_read_chunk()
{
	...
	gIO.seek(gIO.handle, end, IL_SEEK_SET);
...
char * iff_read_data(int size)
{
	...
	if (gIO.read(gIO.handle, buffer, size*sizeof(char), 1) != 1) {
		ifree(buffer);
		return NULL;
	}
...
char *iffReadUncompressedTile(ILushort width, ILushort height, ILbyte depth)
{
	...
	if (gIO.read(gIO.handle, data, tam, 1) != 1) {
		ifree(data);
		return NULL;
	}
