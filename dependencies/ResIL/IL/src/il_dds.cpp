...
ILboolean ilIsValidDds(ILconst_string FileName)
{
	...
	DdsFile = gIO.openReadOnly(FileName);
	if (DdsFile == NULL) {
...
ILboolean iIsValidDds()
{
	...
	iGetDdsHead(&Head);
	gIO.seek(gIO.handle, -(ILint)sizeof(DDSHEAD), IL_SEEK_CUR);  // Go ahead and restore to previous state
...
ILboolean ReadData()
{
	...
	if (Head.Flags1 & DDS_LINEARSIZE) {
		...
		if (gIO.read(gIO.handle, CompData, 1, Head.LinearSize) != (ILuint)Head.LinearSize) {
			ifree(CompData);
			CompData = NULL;
			return IL_FALSE;
		}
		...
		for (z = 0; z < Depth; z++) {
			for (y = 0; y < Height; y++) {
				if (gIO.read(gIO.handle, Temp, 1, Bps) != Bps) {
					ifree(CompData);
					CompData = NULL;
					return IL_FALSE;
				}
