...
ILboolean ilIsValidPcx(ILconst_string FileName)
{
	...
	PcxFile = gIO.openReadOnly(FileName);
	if (PcxFile == NULL) {
...
{
	Head->Manufacturer = gIO.getc(gIO.handle);
	Head->Version = gIO.getc(gIO.handle);
	Head->Encoding = gIO.getc(gIO.handle);
	Head->Bpp = gIO.getc(gIO.handle);
	...
	gIO.read(gIO.handle, Head->ColMap, 1, 48);
	Head->Reserved = gIO.getc(gIO.handle);
	Head->NumPlanes = gIO.getc(gIO.handle);
	Head->Bps = GetLittleUShort();
	Head->PaletteInfo = GetLittleUShort();
	Head->HScreenSize = GetLittleUShort();
	Head->VScreenSize = GetLittleUShort();
	gIO.read(gIO.handle, Head->Filler, 1, 54);
...
ILboolean iIsValidPcx()
{
	...
	if (!iGetPcxHead(&Head))
		return IL_FALSE;
	gIO.seek(gIO.handle, -(ILint)sizeof(PCXHEAD), IL_SEEK_CUR);
...
ILboolean iUncompressPcx(ILimage* image, PCXHEAD *Header)
{
	...
	for (y = 0; y < image->Height; y++) {
		...
		while (x < ScanLineSize) {
			if (gIO.read(gIO.handle, &ByteHead, 1, 1) != 1) {
				goto file_read_error;
			}
			if ((ByteHead & 0xC0) == 0xC0) {
				ByteHead &= 0x3F;
				if (gIO.read(gIO.handle, &Colour, 1, 1) != 1) {
					goto file_read_error;
				}
	...
	if (Header->Version == 5 && image->Bpp == 1) {
		x = gIO.tell(gIO.handle);
		if (gIO.read(gIO.handle, &ByteHead, 1, 1) == 0) {  // If true, assume that we have a luminance image.
	...
	if (Header->NumPlanes == 1 && Header->Bpp == 1) {
		for (j = 0; j < image->Height; j++) {
			...
			while (Bps<Header->Bps) {
				if (gIO.read(gIO.handle, &HeadByte, 1, 1) != 1)
					return IL_FALSE;
				++Bps;
				// Check if we got duplicates with RLE compression
				if (HeadByte >= 192) {
					HeadByte -= 192;
					if (gIO.read(gIO.handle, &Data, 1, 1) != 1)
						return IL_FALSE;
		...
		//		gIO.getc(gIO.handle);	// Skip pad byte
		}
	}
...
ILboolean iSavePcxInternal(ILimage* image)
{
	...
	gIO.putc(0xA, gIO.handle);  // Manufacturer - always 10
	gIO.putc(0x5, gIO.handle);  // Version Number - always 5
	gIO.putc(0x1, gIO.handle);  // Encoding - always 1
	gIO.putc(0x8, gIO.handle);  // Bits per channel
	...
	gIO.putc(0x0, gIO.handle);  // Reserved - always 0

	gIO.putc(image->Bpp, gIO.handle);  // Number of planes - only 1 is supported right now
	...
	gIO.putc(0xC, gIO.handle);  // Pad byte must have this value
	...
	if (TempImage->Format == IL_COLOUR_INDEX) {
		...
		if (TempImage->Pal.PalType == IL_PAL_RGB24) {
			gIO.write(TempImage->Pal.Palette, 1, TempImage->Pal.PalSize, gIO.handle);
		}
		else {
...
ILuint encput(ILubyte byt, ILubyte cnt)
{
	if (cnt) {
		if ((cnt == 1) && (0xC0 != (0xC0 & byt))) {
			if (IL_EOF == gIO.putc(byt, gIO.handle))
				return(0);     /* disk write error (probably full) */
			return(1);
		}
		else {
			if (IL_EOF == gIO.putc((ILubyte)((ILuint)0xC0 | cnt), gIO.handle))
