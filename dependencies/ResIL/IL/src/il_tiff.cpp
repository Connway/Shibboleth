...
ILboolean ilIsValidTiff(ILconst_string FileName)
{
	...
	TiffFile = gIO.openReadOnly((ILstring)FileName);
	if (TiffFile == NULL) {
...
ILboolean ilIsValidTiffFunc()
{
	...
	if (Header1 == MAGIC_HEADER1) {
		...
		gIO.seek(gIO.handle, -4, SEEK_CUR);
	} else if (Header1 == MAGIC_HEADER2) {
		Header2 = GetBigUShort();
		gIO.seek(gIO.handle, -4, SEEK_CUR);
	} else {
		bRet = IL_FALSE;
		gIO.seek(gIO.handle, -2, SEEK_CUR);
	}
...
{
	fd;
	return gIO.read(gIO.handle, pData, 1, tSize);
}
...
{
	fd;
	return gIO.write(pData, 1, tSize, gIO.handle);
}
...
_tiffFileSeekProc(thandle_t fd, toff_t tOff, int whence)
{
	...
	gIO.seek(gIO.handle, tOff, whence);
	return gIO.tell(gIO.handle);
	//return tOff;
}
...
{
	ILint Offset, Size;
	Offset = gIO.tell(gIO.handle);
	gIO.seek(gIO.handle, 0, IL_SEEK_END);
	Size = gIO.tell(gIO.handle);
	gIO.seek(gIO.handle, Offset, IL_SEEK_SET);
