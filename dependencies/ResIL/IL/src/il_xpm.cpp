...
ILboolean ilIsValidXpm(ILconst_string FileName)
{
	...
	XpmFile = gIO.openReadOnly(FileName);
	if (XpmFile == NULL) {
...
ILboolean iIsValidXpm(void)
{
	...
	gIO.seek(gIO.handle, Pos, IL_SEEK_SET);  // Restore position
...
ILint XpmGetsInternal(ILubyte *Buffer, ILint MaxLen)
{
	...
	while ((Current = gIO.getc(gIO.handle)) != IL_EOF && i < MaxLen - 1) {
		if (Current == IL_EOF)
			return 0;
		if (Current == '\n') //unix line ending
			break;
