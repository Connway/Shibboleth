...
ILboolean ilIsValidPic(ILconst_string FileName)
{
	...
	PicFile = gIO.openReadOnly(FileName);
	if (PicFile == NULL) {
...
ILboolean iGetPicHead(PIC_HEAD *Header)
{
	...
	gIO.read(gIO.handle, Header->Comment, 1, 80);
	gIO.read(gIO.handle, Header->Id, 1, 4);
...
ILboolean iIsValidPic()
{
	...
	if (!iGetPicHead(&Head))
		return IL_FALSE;
	gIO.seek(gIO.handle, -(ILint)sizeof(PIC_HEAD), IL_SEEK_CUR);  // Go ahead and restore to previous state
...
ILboolean iLoadPicInternal(ILimage* image)
{
	...
	do {
		...
		Chained = gIO.getc(gIO.handle);
		Channels->Size = gIO.getc(gIO.handle);
		Channels->Type = gIO.getc(gIO.handle);
		Channels->Chan = gIO.getc(gIO.handle);
	...
	Read = readScanlines((ILuint*)image->Data, Header.Width, Header.Height, Channel, Alpha);
...
ILboolean channelReadRaw(ILubyte *scan, ILint width, ILint noCol, ILint *off, ILint bytes)
{
	...
	for (i = 0; i < width; i++) {
		if (gIO.eof(gIO.handle))
			return IL_FALSE;
		for (j = 0; j < noCol; j++)
			if (gIO.read(gIO.handle, &scan[off[j]], 1, 1) != 1)
				return IL_FALSE;
		scan += bytes;
	}
	return IL_TRUE;
...
ILboolean channelReadPure(ILubyte *scan, ILint width, ILint noCol, ILint *off, ILint bytes)
{
	...
	for (i = width; i > 0; ) {
		...
		for (j = 0; j < noCol; j++)
			if (gIO.read(gIO.handle, &col[j], 1, 1) != 1)
				return IL_FALSE;
