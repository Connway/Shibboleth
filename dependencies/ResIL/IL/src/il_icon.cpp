...
ILboolean iIsValidIcon()
{
	...
	ILint read = gIO.read(gIO.handle, &dir, 1, sizeof(dir));
	read += gIO.read(gIO.handle, &entry, 1, sizeof(entry));
	gIO.seek(gIO.handle, -read, SEEK_CUR);
...
ILboolean iLoadIconInternal(ILimage* image)
{
	...
	for (i = 0; i < IconDir.Count; ++i) {
		DirEntries[i].Width = (ILubyte)gIO.getc(gIO.handle);
		DirEntries[i].Height = (ILubyte)gIO.getc(gIO.handle);
		DirEntries[i].NumColours = (ILubyte)gIO.getc(gIO.handle);
		DirEntries[i].Reserved = (ILubyte)gIO.getc(gIO.handle);
	...
	for (i = 0; i < IconDir.Count; i++) {
		gIO.seek(gIO.handle, DirEntries[i].Offset, IL_SEEK_SET);

		// 08-22-2008: Adding test for compressed PNG data
		gIO.getc(gIO.handle); // Skip the first character...seems to vary.
		gIO.read(gIO.handle, PNGTest, 3, 1);
		...
		else
		{
			...
			if (IconImages[i].Head.BitCount < 8) {
				...
				if (gIO.read(gIO.handle, IconImages[i].Pal, IconImages[i].Head.ColourUsed * 4, 1) != 1)
					goto file_read_error;
			}
				...
				if (IconImages[i].Pal == NULL)
					goto file_read_error;
				if (gIO.read(gIO.handle, IconImages[i].Pal, 1, 256 * 4) != 256*4)
					goto file_read_error;
			}
			else {
			...
			if (IconImages[i].Data == NULL)
				goto file_read_error;
			if (gIO.read(gIO.handle, IconImages[i].Data, 1, Size) != Size)
				goto file_read_error;
			...
			if (IconImages[i].AND == NULL)
				goto file_read_error;
			if (gIO.read(gIO.handle, IconImages[i].AND, 1, Size) != Size)
				goto file_read_error;
		}
	}
...
ILint ico_readpng_init(struct IconData* data);
...
ILboolean iLoadIconPNG(struct ICOIMAGE *Icon)
{
#ifndef IL_NO_PNG
	struct IconData data;
	ILint init = ico_readpng_init(&data);
	if (init)
		return IL_FALSE;
...
{
	(void)png_ptr;
	gIO.read(gIO.handle, data, 1, (ILuint)length);
	return;
}
...
ILint ico_readpng_init(struct IconData* data)
{
	...
	png_set_read_fn(data->ico_png_ptr, NULL, ico_png_read);
