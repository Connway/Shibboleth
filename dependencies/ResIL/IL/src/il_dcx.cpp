...
ILboolean ilIsValidDcx(ILconst_string FileName)
{
	...
	DcxFile = gIO.openReadOnly(FileName);
	if (DcxFile == NULL) {
...
ILboolean iIsValidDcx()
{
	...
	if (gIO.read(gIO.handle, &Signature, 1, 4) != 4)
		return IL_FALSE;
	gIO.seek(gIO.handle, -4, IL_SEEK_CUR);
...
ILboolean iLoadDcxInternal()
{
	...
	if (!iIsValidDcx())
		return IL_FALSE;
	gIO.read(gIO.handle, &Signature, 1, 4);

	do {
		if (gIO.read(gIO.handle, &Entries[Num], 1, 4) != 4)
			return IL_FALSE;
		Num++;
	} while (Entries[Num-1] != 0);

	for (i = 0; i < Num; i++) {
		gIO.seek(gIO.handle, Entries[i], IL_SEEK_SET);
		iGetDcxHead(&Header);
		/*if (!iCheckDcx(&Header)) {
...
ILimage *iUncompressDcx(DCXHEAD *Header)
{
	...
	switch (Image->Bpp)
	{
		...
		/*StartPos = gIO.tell(gIO.handle);
		Compressed = (ILubyte*)ialloc(Image->SizeOfData * 4 / 3);
		gIO.read(gIO.handle, Compressed, 1, Image->SizeOfData * 4 / 3);
		...
		ifree(Compressed);
		gIO.seek(gIO.handle, StartPos + Read, IL_SEEK_SET);*/
	...
	// Read in the palette
	if (Image->Bpp == 1) {
		ByteHead = gIO.getc(gIO.handle);	// the value 12, because it signals there's a palette for some reason...
...
ILimage *iUncompressDcxSmall(DCXHEAD *Header)
{
	...
	if (Header->NumPlanes == 1) {
		for (j = 0; j < Image->Height; j++) {
			...
			if (Data != 0)
				gIO.getc(gIO.handle);  // Skip pad byte if last byte not a 0
		}
	}
	else {   // 4-bit images
