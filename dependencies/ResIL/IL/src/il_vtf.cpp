...
ILboolean ilIsValidVtf(ILconst_string FileName)
{
	...
	VtfFile = gIO.openReadOnly(FileName);
	if (VtfFile == NULL) {
...
{
	ILuint read = gIO.read(gIO.handle, Header, 1, sizeof(*Header));

	// @todo: untested endian conversion - I don't have a machine+OS that uses big endian
	#ifdef __BIG_ENDIAN__
	iSwapUInt(Header->Version[0]); // = GetLittleUInt();
	iSwapUInt(Header->Version[1]); //  = GetLittleUInt();
	iSwapUInt(Header->HeaderSize); //  = GetLittleUInt();
	iSwapUShort(Header->Width); //  = GetLittleUShort();
	iSwapUShort(Header->Height); //  = GetLittleUShort();
	iSwapUInt(Header->Flags); //  = GetLittleUInt();
	iSwapUShort(Header->Frames); //  = GetLittleUShort();
	iSwapUShort(Header->FirstFrame); //  = GetLittleUShort();
	iSwapFloat(Header->Reflectivity[0]); //  = GetLittleFloat();
	iSwapFloat(Header->Reflectivity[1]); //  = GetLittleFloat();
	iSwapFloat(Header->Reflectivity[2]); //  = GetLittleFloat();
	iSwapFloat(Header->BumpmapScale); //  = GetLittleFloat();
	iSwapUInt(Header->HighResImageFormat); //  = GetLittleUInt();
	iSwapInt(Header->LowResImageFormat); //  = GetLittleInt();
	#endif
...
ILboolean iIsValidVtf(void)
{
	...
	ILint64 oldReadPos = gIO.tell(gIO.handle);
	ILboolean gotHeader = iGetVtfHead(&Header);
	gIO.seek(gIO.handle, oldReadPos, IL_SEEK_SET);
...
ILboolean iLoadVtfInternal(void)
{
	...
	gIO.seek(gIO.handle, SizeOfData, IL_SEEK_CUR);
			...
			for (Face = 0; Face < NumFaces; Face++) {
				...
				switch (Head.HighResImageFormat)
				{
					...
					case IMAGE_FORMAT_DXT1_ONEBITALPHA:
						...
						if (CompData == NULL)
							return IL_FALSE;
						gIO.read(gIO.handle, CompData, 1, SizeOfData);
					...
					case IMAGE_FORMAT_BGRX8888:
						...
						if (CompData == NULL)
							return IL_FALSE;
						if (gIO.read(gIO.handle, CompData, 1, SizeOfData / 3 * 4) != SizeOfData / 3 * 4) {
							bVtf = IL_FALSE;
							break;
						}
					...
					case IMAGE_FORMAT_RGBA16161616F:
						...
						if (CompData == NULL)
							return IL_FALSE;
						if (gIO.read(gIO.handle, CompData, 1, SizeOfData) != SizeOfData) {
							bVtf = IL_FALSE;
							break;
						}
					...
					case IMAGE_FORMAT_BGRA4444:
						...
						if (CompData == NULL)
							return IL_FALSE;
						if (gIO.read(gIO.handle, CompData, 1, SizeOfData / 2) != SizeOfData / 2) {
							bVtf = IL_FALSE;
							break;
						}
...
/*ILboolean ilSaveVtf(const ILstring FileName)
{
	...
	VtfFile = gIO.openWrite(FileName);
	if (VtfFile == NULL) {
		ilSetError(IL_COULD_NOT_OPEN_FILE);
		return IL_FALSE;
	}
...
ILboolean iSaveVtfInternal()
{
	...
	if (Compression == IL_DXT_NO_COMP) {
		...
		if (gIO.write(TempImage->Data, TempImage->SizeOfData, 1, gIO.handle) != 1)
			return IL_FALSE;
	}
		...
		if (gIO.write(CompData, CompSize, 1, gIO.handle) != 1) {
			ifree(CompData);
			if (TempData != TempImage->Data)
				ifree(TempData);
			return IL_FALSE;
