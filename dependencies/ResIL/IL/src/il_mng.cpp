...
ILboolean ilIsValidMng()
{
	...
	ILint read = gIO.read(gIO.handle, mngSig, 1, sizeof(mngSig));
...
mng_bool MNG_DECL mymngreadstream(mng_handle mng, mng_ptr buffer, mng_size_t size, mng_uint32 *bytesread)
{
	...
	*bytesread = gIO.read(gIO.handle, buffer, 1, (ILuint)size);
...
{
	*byteswritten = gIO.write(buffer, 1, (ILuint)size, gIO.handle);
...
/*ILboolean ilSaveMng(const ILstring FileName)
{
	...
	MngFile = gIO.openWrite(FileName);
	if (MngFile == NULL) {
